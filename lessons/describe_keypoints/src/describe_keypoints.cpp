...
void descKeypoints1()
{
    ...
    cv::Ptr<cv::DescriptorExtractor> descriptorS = cv::xfeatures2d::SIFT::create();
    cv::Mat descSIFT;
    t = (double)cv::getTickCount();
    ...
    // load image from file and convert to grayscale
    cv::Mat imgGray2;
    cv::Mat img2 = cv::imread("../images/img0006.jpg");
    ...
    // t = (double)cv::getTickCount();
    // detectorS->detect(imgGray2, kptsSIFT);
    ...
    // // visualize results
    // cv::Mat visImage3 = img2.clone();
    // cv::drawKeypoints(img2, kptsSIFT, visImage3, cv::Scalar::all(-1), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
    // string windowName3 = "SIFT Results in second";
    // cv::namedWindow(windowName3, 1);
    // imshow(windowName3, visImage3);
