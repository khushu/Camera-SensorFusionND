...
void detKeypoints1()
{
    ...
    cv::Mat imgGray;
    cv::Mat img = cv::imread("../images/img0005.jpg");
    ...
    cv::FAST(imgGray, kptsFAST, threshold, nonmaxSuppression, type );
    time = ((double)cv::getTickCount() - time) / cv::getTickFrequency();
    std::cout << "FAST with n= " << kptsFAST.size() << " keypoints in " << 1000 * time / 1.0 << " ms" << std::endl;
    
    cv::Mat visImageF = img.clone();
    cv::drawKeypoints(img, kptsFAST, visImageF, cv::Scalar::all(-1), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
    string windowNameF = "FAST Results";
    cv::namedWindow(windowNameF, 1);
    imshow(windowNameF, visImageF);
    cv::waitKey(0);
